...
export const DDresponsive= () => {
  return (
    <div className="bg-[#0D0421]   h-[100%] flex flex-col justify-center items-center pb-[20%]  md:hidden">

<section className="bg-[#00000023] flex flex-col   w-full mb-[10%]">
      <Link to='/'>
 <button className='text-white text-lg  ml-2 mt-2 font-medium'>Back</button>
      </Link>

  <div className='w-full flex justify-center items-center'>
  <img src={DD} alt="pf" className='mt-[10%] h-[400px] w-[400px]' />
  </div>

  <br/>
  <h1 className='font-thin text-white text-start text-4xl ml-[10px] '>DRAGONES DORADOS</h1>
  <p className='font-thin text-white text-start text-md ml-[10px] '>Wordpress website for which i was asked to make several changes
and updates</p>
<br/>
<p className='font-thin text-white  text-start text-sm ml-[10px] '>Wordpress, Pagelayer, PHP</p>
<div className=' border-0 border-[#fffffffa] bg-[#fff] text-start  w-full h-[1px] '></div>
  <br/>
<button className='bg-white ml-[10px] flex justify-center items-center w-[20vw] h-[5vh] font-thin rounded-sm text-md hover:bg-transparent hover:text-white hover:border hover:border-[#fff] '> <MdRemoveRedEye className='h-[20px] w-[20px] mr-[2px] hover:text-white'/> Website</button>
  </section>
